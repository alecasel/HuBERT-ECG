...
class CinC2020(Metric):
    ...
    def build_conditions(self, conditions):
        ...
        for i, condition in enumerate(conditions):
            if condition in MAPPING.keys():
                # rename conditions in self.conditions based on MAPPING
                conditions[i] = MAPPING[condition]
        return conditions
    ...
    def compute(self):
        ...
        assert len(gt) == len(preds), "gt and preds must have the same shape. Found {} and {}".format(
            gt.shape, preds.shape)
        assert gt.size(1) == len(self.conditions), "Number of conditions in gt and preds must be the same. Found {} and {}".format(
            gt.shape[1], len(self.conditions))
        ...
        self.conditions = [
            c for c in self.conditions if c in self.scored_conditions]
        ...
        assert gt.shape[1] == len(self.conditions), "Number of conditions in gt and preds cols must be the same. Found {} and {}".format(
            gt.shape[1], len(self.conditions))
        assert gt.shape == preds.shape, "gt and preds must have the same shape. Found {} and {}".format(
            gt.shape, preds.shape)
        ...
        normal_index = task_weights.columns.tolist().index(
            normal_class) if normal_class in self.conditions else None

        # check that column and index order is preserved
        assert task_weights.index.tolist() == self.conditions, "task_weights index must be the same as conditions. Found {} and {}".format(
            task_weights.index.tolist(), self.conditions)
        assert task_weights.columns.tolist() == self.conditions, "task_weights columns must be the same as conditions. Found {} and {}".format(
            task_weights.columns.tolist(), self.conditions)

        task_weights = task_weights.to_numpy(dtype=float)

        assert task_weights.shape == (len(self.conditions), len(
            self.conditions)), "task_weights must have shape len(conditions) x len(conditions). Found {}".format(task_weights.shape)

        task_weights = torch.from_numpy(task_weights).to(device)

        modified_confusion_matrix = compute_modified_confusion_matrix(
            gt, preds)
        ...
        if normal_index is not None:
            ...
            modified_confusion_matrix = compute_modified_confusion_matrix(
                gt, inactive_outputs)
            inactive_score = np.nansum(
                task_weights * modified_confusion_matrix)
        else:
            inactive_score = 0.0
