...
import numpy as np
from hubert_ecg import HuBERTECGConfig
add_safe_globals([np.core.multiarray.scalar, HuBERTECGConfig])
...
def test(args, model: nn.Module, metrics: Iterable[nn.Module]):
    ...
    for batch_id, (ecg, _, labels) in enumerate(tqdm(dataloader, total=len(dataloader))):
        ...
        ecgs = [random_crop(ecg, 2500//args.downsampling_factor)
                for _ in range(args.n_augs)] if args.tta else [ecg]

        probs = []
        for aug_ecg in ecgs:  # iterate over augmented batches
            # forward with a single augmented batch
            with torch.no_grad():
                out = model(aug_ecg, attention_mask=None, output_attentions=False,
                            output_hidden_states=False, return_dict=True)
                if isinstance(model, HuBERT):
                    logits = model.logits(
                        out['last_hidden_state']).transpose(1, 2)
                elif isinstance(model, HuBERTClassification):
                    logits = out[0]
            # end of forward

            probs.append(torch.sigmoid(logits) if args.task == 'multi_label' else torch.softmax(
                logits, dim=-1))  # [BS x num_labels] * N_AUGS

        # average probs over augmented batches for tta
        probs = torch.stack(probs).mean(
            dim=0) if args.tta_aggregation == 'mean' else torch.stack(probs).max(dim=0).values

        if args.save_probs:
            dest_path = f"probs/{os.path.basename(args.path_to_dataset_csv)[:-4]}"
            os.makedirs(dest_path, exist_ok=True)
            np.save(os.path.join(dest_path, f"probs_{batch_id}_bs{args.batch_size}"), probs.cpu(
            ).numpy())  # if no shuffle, then batch_id is the index of the batch in the dataset
            logger.info(
                f"Probs saved at probs/{os.path.basename(args.path_to_dataset_csv)[:-4]}/probs_{batch_id}_bs{args.batch_size}.npy")
    ...
    performance = pd.DataFrame(
        columns=testset.diagnoses_cols, index=list(metrics.keys()))

    for name, metric in metrics.items():
        score = metric.compute()  # compute metric over all test set
        print(f"{name} = {score}, macro: {score.mean()}")
        performance.loc[name] = score.cpu().numpy(
        ) if args.task == 'multi_label' else score.mean().cpu().numpy()
    ...
    if args.save_id is not None:
        if args.tta_aggregation == 'max':
            performance.to_csv(
                f"./performance/performance_{args.save_id}_max.csv")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("ecg_dir_path", type=str,
                        help="Path to directory with ecg files")

    parser.add_argument("batch_size", type=int, help="Batch size")

    parser.add_argument("model_path", type=str,
                        help="Path to a model checkpoint that is to be tested")

    parser.add_argument("--downsampling_factor", type=int,
                        help="Downsampling factor")

    parser.add_argument("--label_start_index", default=3, type=int,
                        help="Label start index in dataframe. Default 3")

    parser.add_argument("--save_id", type=str, default=None,
                        help="The id to use for saving the csv file of performance metrics")

    parser.add_argument("--tta", default=False, action="store_true",
                        help="Whether to use test time augmentation")

    parser.add_argument("--tta_aggregation", type=str, default='mean',
                        choices=['mean', 'max'], help="Aggregation method for tta")

    parser.add_argument("--n_augs", type=int, default=3,
                        help="Number of augmentations")

    parser.add_argument("--challenge_metric", default=False,
                        action="store_true", help="Whether to compute CinC2020 metric")

    parser.add_argument("--task", type=str, default='multi_label',
                        choices=['multi_label', 'multi_class', 'regression'], help="Task type")

    parser.add_argument("--save_probs", default=False,
                        action="store_true", help="Whether to save probs")
    ...
    logger.info(
        f"Loading finetuned model from {args.model_path.split('/')[-1]}")

    cpu_device = torch.device('cpu')

    try:
        checkpoint = torch.load(
            args.model_path, map_location=cpu_device, weights_only=False)
    except TypeError:
        # per compatibilità con torch < 2.6
    ...
    if type(config) == HubertConfig:
        config = HuBERTECGConfig(ensemble_length=1, vocab_sizes=[
                                 100], **config.to_dict())
    ...
    if checkpoint['linear']:
        ...
        classifier_hidden_size = checkpoint['model_state_dict'][keys[-2]
                                                                ].size(-1)
    ...
    hubert.load_state_dict(new_model_state_dict, strict=False)
