...
def train(args):
    ...
    wandb.init(entity="my-entity", project="my-project",
    ...
    if args.resume_pretraining:
        ...
        checkpoint = torch.load(
            args.load_path, map_location=torch.device('cpu'))

        config = checkpoint['model_config']
        assert checkpoint['pretraining_vocab_sizes'] == args.vocab_sizes
        if type(config) == HubertConfig:
            config = HuBERTECGConfig(ensemble=len(
                checkpoint['pretraining_vocab_sizes']), vocab_sizes=checkpoint['pretraining_vocab_sizes'], **config.to_dict())
        ...
        if args.train_iteration != previous_iteration:  # when switching to subsequent training iterations
            logger.info(
                "Switching to another pretraining iteration: changing label embedding and restoring dropouts...")
            hubert.label_embedding = nn.ModuleList(nn.Embedding(
                vocab_size, hubert.config.classifier_proj_size) for vocab_size in args.vocab_sizes)
        ...
        best_val_loss = checkpoint['best_val_loss'] if args.train_iteration == previous_iteration else float(
            'inf')
        ...
        # don't load state dict when switching to subsequent train iterations
        if args.train_iteration == previous_iteration:
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            optimizer.param_groups[0]['weight_decay'] = max(
                0.01, optimizer.param_groups[0]['weight_decay'])
            for name, module in hubert.named_modules():
                if 'dropout' in name:
        ...
        lr_scheduler = get_linear_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=ceil(
            0.08*args.training_steps), num_training_steps=args.training_steps)

    scaler = amp.GradScaler() if USE_CUDA else None

    # number of params
    logger.info(
        f"Number of parameters: {sum(p.numel() for p in hubert.parameters())}")
    ...
    assert len(
        args.vocab_sizes) == train_set.ensamble_length, f"len(vocab_sizes) must be equal to the number of tasks. Found {len(args.vocab_sizes)} and {train_set.ensamble_length} tasks"
    for v, k in zip(args.vocab_sizes, train_set.ensamble_kmeans):
        assert v == k.cluster_centers_.shape[
            0], f"vocab_sizes must be equal to the number of clusters in the kmeans models. Found {v} and {k.cluster_centers_.shape[0]} clusters"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for ecg, attention_mask, ensamble_labels in tqdm(train_dl, total=len(train_dl)):
            ...
            with autocast():
                ...
                out_encoder_dict = hubert(ecg, attention_mask=attention_mask,
                                          output_attentions=False, output_hidden_states=False, return_dict=True)
                # logger.info("Computed encodings")

                mask = out_encoder_dict['mask_time_indices']

                ensamble_logits = hubert.logits(
                    out_encoder_dict['last_hidden_state'])
                ...
                assert len(ensamble_labels) == len(
                    ensamble_logits), f"len(ensamble_labels) must be equal to len(ensamble_logits). Found {len(ensamble_labels)} and {len(ensamble_logits)}"
            ...
            if global_step % args.val_interval == 0:
                ...
                for ecg, _, ensamble_labels in tqdm(val_dl, total=len(val_dl)):
                    ...
                    ensamble_labels = ensamble_labels.transpose(
                        0, 1)  # (ensamble_length, BS, F)

                    with torch.no_grad():
                        out_encoder_dict = hubert(
                            ecg, attention_mask=None, output_attentions=False, output_hidden_states=False, return_dict=True)
                        ensamble_logits = hubert.logits(
                            out_encoder_dict['last_hidden_state'])

                        assert len(ensamble_labels) == len(
                            ensamble_logits), f"VAL! len(ensamble_labels) must be equal to len(ensamble_logits). Found {len(ensamble_labels)} and {len(ensamble_logits)}"
                        ...
                        for labels, logits in zip(ensamble_labels, ensamble_logits):
                            ...
                            # mean over batch for a given task
                            accuracy += (logits.argmax(dim=1) ==
                                         labels).float().mean()
                ...
                if val_loss <= best_val_loss - MINIMAL_IMPROVEMENT:  # if loss improves significantly, save checkpoint
                    ...
                    torch.save(
                        checkpoint, SELF_SUPERVISED_MODEL_CKPT_PATH + checkpoint_name)

                    logger.info(
                        f"New best (best_val_loss = {best_val_loss}) - model saved at step {global_step}")

                    # unburdening model from regularization
                    dynamic_regularizer(
                        optimizer, hubert, penalty=False) if args.dynamic_reg else None

                # if loss doesn't improve significantly but accuracy does, save checkpoint anyway
                    ...
                    torch.save(
                        checkpoint,  SELF_SUPERVISED_MODEL_CKPT_PATH + checkpoint_name)
                    logger.info(
                        f"Val loss not improved but val accuracy did (best_val_accuracy = {best_val_accuracy}) - model saved at step {global_step}")
                    ...
                    if args.dynamic_reg and patience_count % (patience // args.intervals_for_penalty) == 0 and patience_count != patience:
                        # penalizing model with regularization
                        dynamic_regularizer(optimizer, hubert, penalty=True)

                    if patience_count == patience:
                        logger.warning(
                            f"EARLY STOPPING: Max num of val intervals with no improvement reached at {global_step}")
                        wandb.log({
                            "patience_count": patience_count
                        })
                        return
    ...
    logger.info(
        f"STATS: Global step={global_step}, Best val loss={best_val_loss}")
    wandb.finish()
...
if __name__ == "__main__":
    ...
    if args.epochs is not None and args.training_steps is not None:
        raise ValueError(
            "Argument epochs and training_steps cannot be provided at the same time")
    ...
    if args.largeness not in ["base", "large", "small"]:
        raise ValueError(
            f"Argument largeness must be in [base, large, x-large] range. Inserted {args.largeness}")

    if args.mask_time_prob < 0.0 or args.mask_time_prob > 1.0:
        raise ValueError(
            f"Argument mask_time_prob must be a float in [0.0, 1.0] range. Inserted {args.mask_time_prob}")

    if args.alpha < 0.0 or args.alpha > 1.0:
        raise ValueError(
            f"Argument alpha must be a float in [0.0, 1.0] and must provided if pretrain is provided. Inserted {args.alpha}")

    if not os.path.exists(args.kmeans_path):
        raise ValueError(
            f"Argument kmeans_path must be a valid path. Inserted {args.kmeans_path}")

    if not os.path.exists(args.train_features_path):
        raise ValueError(
            f"Argument train_features_path must be a valid path. Inserted {args.train_features_path}")

    if not os.path.exists(args.val_features_path):
        raise ValueError(
            f"Argument val_features_path must be a valid path. Inserted {args.val_features_path}")

    if args.resume_pretraining and args.load_path is None:
        raise ValueError(
            "Argument load_path must be provided is argument resume_pretraining is provided")
