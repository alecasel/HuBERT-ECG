...
def get_CI_intervals_by_bootstrapping(path_to_csv_test_set, label_start_index=3, N=1000, task='multi_label', average="none", alpha=0.95, path_to_performance=None):
    ...
    probs_dir = os.path.join(
        f"probs/{os.path.basename(path_to_csv_test_set)[:-4]}")
    probs = [os.path.join(probs_dir, prob)
             for prob in os.listdir(probs_dir) if prob.endswith(".npy")]
    ...
    # Sanity checks
    assert probs.shape == (len(
        df), num_labels), f"Mismatch in shape: expected ({len(df)}, {num_labels}), got {probs.shape}"
    ...
    if path_to_performance is None:
        ...
        # metrics name as index, labels as columns
        point_estimate = pd.read_csv(path_to_performance, index_col=0)
    ...
    # bootstrapping
    bootstrapped_differences = {metric_name: []
    ...
    for i in range(N):
        ...
        for metric_name, metric in metrics.items():
            ...
            # numpy array of shape (num_classes,) if average is "none", else a scalar numpy array
            val = metric.compute().cpu().numpy()
            # list of N numpy arrays of shape (num_classes,) if average is "none", else a list of N scalar numpy arrays
            bootstrapped_differences[metric_name].append(
                val - point_estimate[metric_name])
    ...
    for metric_name in metrics.keys():
        # transforms list of differences into numpy array of differences
        diffs = np.array(bootstrapped_differences[metric_name])

        # Compute percentiles correctly based on whether we have class-wise metrics or single values
        lower_percentile = np.percentile(
            diffs, lower_bound, axis=0 if average == "none" else None)
        upper_percentile = np.percentile(
            diffs, upper_bound, axis=0 if average == "none" else None)
...
def multilabel_split(path_to_csv_file, test_size, val_size, fold='', random_state=None, label_start_index=3, save=False):
    ...
    if save:
        train.to_csv(path_to_csv_file[:-4] + "_" +
                     fold + "_train.csv", index=False)
        val.to_csv(path_to_csv_file[:-4] + "_" +
                   fold + "_val.csv", index=False)
        test.to_csv(path_to_csv_file[:-4] + "_" +
                    fold + "_test.csv", index=False)
...
def prepare_data_ptb_xl(min_cnt=1):
    ...
    ptb_all['filename'] = ptb_all['filename'].apply(
    ...
    ptb_form['filename'] = ptb_form['filename'].apply(
        lambda x: "HR" + os.path.basename(x).replace("_hr", ".hea.npy"))
    ...
    ptb_rhythm['filename'] = ptb_rhythm['filename'].apply(
        lambda x: "HR" + os.path.basename(x).replace("_hr", ".hea.npy"))
    ...
    ptb_diag['filename'] = ptb_diag['filename'].apply(
        lambda x: "HR" + os.path.basename(x).replace("_hr", ".hea.npy"))
    ...
    ptb_diag_subclass['filename'] = ptb_diag_subclass['filename'].apply(
        lambda x: "HR" + os.path.basename(x).replace("_hr", ".hea.npy"))

    # PTB DIAG SUPERCLASS
    ptb_diag_superclass = df_ptb_xl[[
        'filename_hr', 'age', 'sex', 'strat_fold']]
    ...
    ptb_diag_superclass.rename(
        columns={'filename_hr': 'filename'}, inplace=True)
    ptb_diag_superclass['filename'] = ptb_diag_superclass['filename'].apply(
        lambda x: "HR" + os.path.basename(x).replace("_hr", ".hea.npy"))
...
def ecg_preprocessing(ecg_signal, original_frequency, target_frequency=100, band_pass=[0.05, 47]):
    ...
    ecg_signal = signal.resample(ecg_signal, int(ecg_signal.shape[-1] * (500/original_frequency)), axis=1)
    ...
    # this band focuses on dominant component of ecg waves
