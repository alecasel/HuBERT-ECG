...
class ECGDataset(Dataset):
    def __init__(
        ...
        self.ecg_dataframe = pd.read_csv(
            path_to_dataset_csv, dtype={'filename': str})
        ...
        if pretrain:
            ...
            kmeans_paths = [
                path for path in kmeans_paths if not path.startswith("#")]

            self.ensamble_length = len(kmeans_paths)
            self.ensamble_kmeans = [joblib.load(
                path.strip()) for path in kmeans_paths]
            self.features_path = features_path
        elif not encode:
            self.diagnoses_cols = self.ecg_dataframe.columns.values.tolist()[
                label_start_index:]
            assert len(
                self.diagnoses_cols) > 0, "No labels found in the dataset"
    ...
    def compute_weights(self):
        ...
        if len(self.diagnoses_cols) > 1:
            ...
            for label in self.diagnoses_cols:
                ...
                weight = (self.ecg_dataframe.__len__() - count) / \
                    (count + 1e-9)
                weights.append(weight)
        else:
            num_labels = self.ecg_dataframe[self.diagnoses_cols[0]].max() + 1
            weights = num_labels / \
                self.ecg_dataframe[self.diagnoses_cols].value_counts()
    ...
    def __getitem__(self, idx):
        ...
        ecg_path = ecg_filename if os.path.isfile(
            ecg_filename) else os.path.join(self.ecg_dir_path, ecg_filename)
        ...
        if not self.encode:
            if self.beat_based_attention_mask:
                attention_mask = self.compute_beat_based_attention_mask(
                    ecg_data)
            else:
                attention_mask = self.compute_attention_mask_for_padding(
                    ecg_data)
        ...
        if self.pretrain:
            ...
            labels = [kmeans.predict(features).tolist()
                      for kmeans in self.ensamble_kmeans]
            ...
            output = (
                torch.from_numpy(ecg_data.copy()).float(),
                torch.from_numpy(attention_mask.copy()).long(),
                torch.from_numpy(labels.copy()).float() if len(
                    self.diagnoses_cols) > 1 else torch.from_numpy(labels.copy()).long()
            )
    ...
    def compute_beat_based_attention_mask(self, ecg_data):
        ...
        # list of couples <start, stop> for each P wave detected
        p_starts_stops = np.array(p_starts_stops).reshape(-1, 2)

        t_starts_stops = t_wave[t_wave != 0].index.tolist()
        if len(t_starts_stops) % 2 != 0:
            t_starts_stops.append(min(t_starts_stops[-1]+1, 2499))
        # list of couples <start, stop> for each T wave detected
        t_starts_stops = np.array(t_starts_stops).reshape(-1, 2)
        ...
        # list of couples <start, stop> for each QRS complex detected
        qrs_starts_stops = np.array(qrs_starts_stops).reshape(-1, 2)
        ...
        # since the leads are temporally aligned, interest regions should be located within the same intervals
        attention_mask = np.repeat([attention_mask], 12, axis=0)
        attention_mask = np.concatenate(attention_mask, axis=0)
