...
SUPERVISED_MODEL_CKPT_PATH = "F:/models/checkpoints_samitrop/"
...
def finetune(args):
    ...
    autocast = amp.autocast if USE_CUDA else nullcontext
    ...
    train_pos_weights = train_set.weights.to(
        device) if args.use_loss_weights else None
    ...
    val_pos_weights = val_set.weights.to(
        device) if args.use_loss_weights else None

    num_workers = 0 if device.type == 'cpu' else 6

    train_dl = DataLoader(
        train_set, collate_fn=train_set.collate,
        num_workers=0, batch_size=args.batch_size,
        shuffle=True, pin_memory=False, drop_last=True)

    val_dl = DataLoader(
        val_set, collate_fn=val_set.collate,
        num_workers=0, batch_size=args.batch_size,
    ...
    args.val_interval = len(
        train_dl) if args.val_interval is None else args.val_interval
    ...
    if args.resume_finetuning:
        ...
        logger.info(
            f"Loading pretraining checkpoint {args.load_path.split('/')[-1]} to resume finetuning")

        try:
            checkpoint = torch.load(
                args.load_path, map_location='cpu', weights_only=False)
        except TypeError:
        ...
        hubert = HuBERTClassification(pretrained_hubert, num_labels=args.finetuning_vocab_size,
                                      classifier_hidden_size=args.classifier_hidden_size, use_label_embedding=args.use_label_embedding)
        hubert.to(device)

        # strict false prevents errors when trying to match mask token key
        ...
        if args.freezing_steps is not None and global_step < args.freezing_steps:
            ...
            hubert.set_transformer_blocks_trainable(
                n_blocks=args.transformer_blocks_to_unfreeze)
        ...
        if args.layer_wise_lr and all(p.requires_grad for p in hubert.hubert_ecg.encoder.layers.parameters()):
            parameters_group.append(
                {"params": hubert.hubert_ecg.feature_projection.parameters(), "lr": 1e-7})
            parameters_group.append(
                {"params": hubert.hubert_ecg.encoder.layers[:args.transformer_blocks_to_unfreeze-4].parameters(), "lr": 1e-7})
            parameters_group.append(
                {"params": hubert.hubert_ecg.encoder.layers[args.transformer_blocks_to_unfreeze-4:].parameters(), "lr": lr})
            parameters_group.append(
                {"params": hubert.classifier.parameters(), "lr": 1e-5})
        else:
            parameters_group.append(
                {"params": filter(lambda p: p.requires_grad, hubert.parameters()), "lr": lr})
        ...
        lr_scheduler = get_linear_schedule_with_warmup(
            optimizer=optimizer,
            num_warmup_steps=ceil(
                args.ramp_up_perc*(args.training_steps-global_step)),
        ...
        if args.downsampling_factor is None:
            ...
            raise ValueError(
                f"Downsampling factor {args.downsampling_factor} not supported")
        ...
        logger.info(
            f"Loading pretraining checkpoint {args.load_path.split('/')[-1]} to start finetuning")
        ...
        pretrained_hubert = HuBERT(config)
        pretrained_hubert.load_state_dict(
            checkpoint['model_state_dict'])  # load backbone weights
        ...
        hubert = HuBERTClassification(pretrained_hubert, num_labels=args.vocab_size,
                                      classifier_hidden_size=args.classifier_hidden_size,  use_label_embedding=args.use_label_embedding)
        hubert.to(device)
        ...
        if args.freezing_steps is not None:
            hubert.set_transformer_blocks_trainable(
                n_blocks=0)  # freeze all transformer blocks
            hubert.set_feature_extractor_trainable(
                False)  # freeze conv feature extractor
        else:
            # makes trainable only the last n_blocks of the transformer encoder
            hubert.set_transformer_blocks_trainable(
                n_blocks=args.transformer_blocks_to_unfreeze)
            hubert.set_feature_extractor_trainable(
                args.unfreeze_conv_embedder)  # frozen by default
    ...
    epochs = args.training_steps // (len(train_dl) // accumulation_steps) + \
        1 if args.training_steps is not None else args.epochs

    start_epoch = global_step // len(train_dl)

    task2metric = {
        'multi_label': {
            "f1_score": F1_score(num_labels=args.vocab_size, average=None),
    ...
    assert args.target_metric in metrics.keys(
    ), f"Target metric {args.target_metric} not available for task {args.task}"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for ecg, attention_mask, labels in tqdm(train_dl, total=len(train_dl)):
            ...
            with autocast():
                logits, _ = hubert(ecg, attention_mask=attention_mask,
            ...
            if global_step % args.val_interval == 0:
                ...
                for ecg, _, labels in tqdm(val_dl, total=len(val_dl)):
                    ...
                    with torch.no_grad():
                        logits, _ = hubert(
                            ecg, attention_mask=None, output_attentions=False, output_hidden_states=False, return_dict=True)
                ...
                if val_loss <= best_val_loss - MINIMAL_IMPROVEMENT:
                    ...
                    torch.save(checkpoint, os.path.join(
                        SUPERVISED_MODEL_CKPT_PATH, args.sweep_dir, checkpoint_name))

                    logger.info(
                        f"New best val loss = {best_val_loss}. Checkpoint saved at step {global_step}")

                    dynamic_regularizer(optimizer=optimizer, model=hubert,
                                        penalty=False) if args.dynamic_reg else None  # reward for loss
                    ...
                    logger.info(
                        f"Val loss not improved but {args.target_metric} did (= {target_score}). Checkpoint saved at step {global_step}")

                    # reward for target metric
                    dynamic_regularizer(
                        optimizer=optimizer, model=hubert, penalty=False) if args.dynamic_reg else None
                    ...
                    if args.dynamic_reg and patience_count % (args.patience // args.intervals_for_penalty) == 0 and patience_count != args.patience:
                        # penalize model with regularization but not too much
                        dynamic_regularizer(
                            optimizer=optimizer, model=hubert, penalty=True)
    ...
    logger.info(
        f"Best val target score = {best_val_target_score}, ({args.target_metric})")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["f1_score", "recall", "precision",
                 "specificity", "auroc", "auprc", "accuracy"]
    )
    ...
    if args.train_iteration > 3 or args.train_iteration < 1:
        raise ValueError(
            f"Argument train_iteration must be an integer in [1, 3] range. Inserted {args.train_iteration}")

    if args.training_steps is None and args.epochs is None:
        raise ValueError(
            "One argument between training_steps and epochs must be provided")

    if args.training_steps is not None and args.val_interval is None:
        raise ValueError(
            "Argument val_interval must be provided if argument training_steps is provided")

    if args.training_steps is not None and args.training_steps % args.val_interval != 0:
        raise ValueError(
            f"Argument training_steps must be divisible by argument val_interval. Inserted {args.training_steps} and {args.val_interval}")

    if args.ramp_up_perc < 0 or args.ramp_up_perc > 1:
        raise ValueError(
            "Argument ramp_up_perc must be a float in [0, 1] range")

    if args.random_init and args.resume_finetuning:
        raise ValueError(
            "Arguments random_init and resume_finetuning cannot be provided together")

    if (args.resume_finetuning or args.random_init == False) and args.load_path is None:
        raise ValueError(
            "Argument load_path must be provided when start/resume finetuning")

    if args.freezing_steps is not None and args.freezing_steps > args.training_steps:
        raise ValueError(
            "Argument freezing_steps cannot be greater than argument training steps")
    ...
    if args.accumulation_steps is not None and args.training_steps is not None and args.training_steps % args.accumulation_steps != 0:
        raise ValueError(
            "Argument training_steps must be divisible by argument accumulation_steps")

    if args.random_init and args.largeness is None:
        raise ValueError(
            "Argument largeness must be provided if argument random_init is provided")

    if args.random_init and args.load_path is not None:
        logger.warning(
            "Argument random_init is provided. Argument load_path will be ignored")

    if args.dynamic_reg and args.patience < args.intervals_for_penalty:
        raise ValueError(
            f"Argument patience must be greater or equal to argument intervals_for_penalty when using dynamic_reg. Setting patience = {args.intervals_for_penalty}")
