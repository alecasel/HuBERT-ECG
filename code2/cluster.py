...
def cluster(args):
    ...
    while n_clusters <= args.n_clusters_end:
        ...
        for _, filenames in tqdm(dataloader, total=len(dataloader)):
            ...
            features = [np.load(os.path.join(args.in_dir, filename))
        ...
        if args.train_iteration == 1:
            ...
            model_name = "k_means_" + \
                str(n_clusters) + "_encoder_" + str(args.layer) + \
                "_" + str(args.train_iteration)
...
def evaluate_clustering(args):
    ...
    logger.info(
        f"Ready to evaluate clustering model {args.model_path.split('/')[-1]}")
    ...
    for _, filenames in tqdm(dataloader, total=len(dataloader)):
        ...
        # (BS * 93, ) containing values in [0, n_clusters - 1]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Cluster ECG features or representations")
